...
public class Queue {
	...
	**/
	public QNode dequeue() {
		...
		numElements--;
		return retNode;	// remove this line once the funciton is completed.
	}
	...
	**/
	private void copyArray(QNode [] array) {
	    ...
	    QNode[] newQueue = new QNode[size];
	    for( int i = 0; i <= size; i++ )
        {
	        
           newQueue[i] = new QNode(queue[i]);
        }
	    queue = newQueue;
	    end = size - 1;
	}
